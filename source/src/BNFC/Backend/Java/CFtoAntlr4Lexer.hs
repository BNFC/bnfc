...
lexSymbols ss = vcat $  map transSym ss
  where
    transSym (s,r) =
     "surrogate_id "<> text r <>  " : " <> text (escapeChars s) <> " ;"
...
lexMultiComment (b,e) =
    "MULTICOMMENT : '" cstring b "' (.)*? '" cstring e "' -> skip;"