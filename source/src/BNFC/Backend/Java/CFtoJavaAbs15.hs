...
cf2JavaAbs :: String -> String -> CF -> Bool -> [(FilePath, String)]
cf2JavaAbs _ packageAbsyn cf ln =
    concatMap (prData ln header packageAbsyn user) rules
 where
...
prData ln header packageAbsyn user (cat, rules) =
  categoryClass ++ mapMaybe (prRule ln header packageAbsyn funs user cat) rules
      where
      funs = map fst rules
      categoryClass
...
prRule :: Bool     -- ^ Include line number info in generated classes.
       -> String   -- ^ Header
...
prRule ln h packageAbsyn funs user c (fun, cats)
...
-- >>> prInstVars False [("A",1,""), ("B",1,""), ("A",2,"abc")]
-- public final A _1, abc_2;
-- public final B _1;
-- >>> prInstVars True [("A",1,""), ("B",1,""), ("A",2,"abc")]
-- public final A _1, abc_2;
-- public final B _1;
-- public int line_num, col_num, offset;
prInstVars :: Bool -> [IVar] -> Doc
prInstVars ln [] = case ln of
  True -> "public int line_num, col_num, offset;"
  False -> empty
prInstVars ln vars@((t,_,_):_) =
    "public" <+> "final" <+> text t <+> uniques <> ";" $$ prInstVars ln vs'
 where
