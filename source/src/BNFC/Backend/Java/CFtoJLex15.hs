...
import BNFC.Options -- TODO maybe this is not supposed to be seen from here

--The environment must be returned for the parser to use.
cf2jlex :: String -> CF -> JavaLexerParser -> (Doc, SymEnv)
cf2jlex packageBase cf jlexerparser = (vcat
 [
  prelude jlexerparser packageBase,
  cMacros,
  lexSymbols jlexerparser env,
  restOfJLex cf
 ], env)
  where
...
-- | File prelude
prelude :: JavaLexerParser -> String -> Doc
prelude jflex packageBase = vcat
    ...
    , nest 2 $ vcat
        ...
        , "public String buff()" <+> braces
            (case jflex of
                JFlexCup -> "return new String(zzBuffer,zzCurrentPos,10).trim();"
                JLexCup  -> "return new String(yy_buffer,yy_buffer_index,10).trim();"
                Antlr4   -> "return null;"
            )
        ]
    , "%}"
...
lexSymbols jlexerparser ss = vcat $  map transSym ss
  where
    transSym (s,r) =
    ...
    escapeChars = concatMap (escapeChar jlexerparser)
