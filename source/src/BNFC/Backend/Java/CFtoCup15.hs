...
cf2Cup :: ToolParameters -> CF -> SymEnv -> String
cf2Cup tpar cf env = unlines
    [
     header,
     declarations (packageAbsyn tpar) (allCats cf),
     tokens env,
     specialToks cf,
     specialRules cf,
     prEntryPoint cf,
     prRules (rulesForCup (packageAbsyn tpar) cf env)
    ]
    where
      header :: String
      header = unlines
          ["// -*- Java -*- This Cup file was machine-generated by BNFC"
          , "package" +++ (packageBase tpar) ++ ";"
          , ""
          , "parser code {:"
          , parseMethod (packageAbsyn tpar) (firstEntry cf)
          , "public <B,A extends java.util.LinkedList<? super B>> "
            ++ "A cons_(B x, A xs) { xs.addFirst(x); return xs; }"
          , definedRules (packageAbsyn tpar) cf
...
constructRule packageAbsyn cf env rules nt =
    (nt, [ (p, generateJavaAction packageAbsyn nt (funRule r) (revM b m) b)
          | r0 <- rules,
...
generateJavaAction :: String -> NonTerminal -> Fun -> [MetaVar]
               -> Bool   -- ^ Whether the list should be reversed or not.
                         --   Only used if this is a list rule.
               -> Action
generateJavaAction packageAbsyn nt f ms rev
