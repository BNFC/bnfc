...
cf2Abstract :: Bool -> Bool -> Bool -> String -> CF -> String
cf2Abstract byteStrings dataTypeable generics name cf = unlines $
  (if dataTypeable then "{-# LANGUAGE DeriveDataTypeable #-}" else "") :
  (if generics then "{-# LANGUAGE DeriveGeneric #-}" else "") :
  ("module "++name +++ "where\n") :
  "-- Haskell module generated by the BNF converter\n" :
  (if byteStrings then "import qualified Data.ByteString.Char8 as BS" else "") :
  (if dataTypeable then "import Data.Data (Data,Typeable)" else "") :
  (if generics then "import GHC.Generics (Generic)" else "") :
  ...
  where
    derivingClasses = prParenth . intercalate "," $ concat
        [ ["Eq","Ord","Show"]
        , (if dataTypeable then ["Data","Typeable"] else [])
        , (if generics then ["Generic"] else [] ) ]
