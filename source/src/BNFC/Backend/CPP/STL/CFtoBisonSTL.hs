...
cf2Bison :: RecordPositions -> Maybe String -> CF -> SymMap -> String
cf2Bison rp inPackage cf env
 = unlines
    [header inPackage cf,
...
header :: Maybe String -> CF -> String
header inPackage cf = unlines
    ...
    , unlines $ map (parseMethod cf inPackage) eps
    , nsEnd inPackage
    , "%}"
    ]
  where
...
parseMethod :: CF -> Maybe String -> Cat -> String
parseMethod cf inPackage cat = unlines $ concat
  [ [ cat' ++ "* p" ++ par ++ "(FILE *inp)"
    , "{"
