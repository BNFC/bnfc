...
cpptest inPackage cf =
  unlines
   [
    "/*** Compiler Front-End Test automatically generated by the BNF " ++
      "Converter ***/",
    "/*                                                              " ++
      "            */",
    "/* This test will parse a file, print the abstract syntax tree, " ++
      "and then    */",
    ...
    "  " ++ scope ++ def ++ " *parse_tree = " ++ scope ++ "p" ++ def ++ 
      "(input);",
    "  if (parse_tree) {",
    "    printf(\"\\nParse Succesful!\\n\");",
    "    if (!quiet) {",
...
mkHeaderFile inPackage cf cats eps env = unlines
 [
  ...
  "extern " ++ nsScope inPackage ++ "YYSTYPE " ++ nsString inPackage ++ 
    "yylval;",
  "",
  "#endif"
 ]
 where
  ...
  mkForwardDec _ = ""
  mkVar s | (normCat s == s) = "  " ++ (identCat s) ++"*" +++
    (map toLower (identCat s)) ++ "_;\n"
  mkVar _ = ""
  mkDefines n [] = mkString n
  mkDefines n ((_,s):ss) = ("#define " ++ s +++ (show n) ++ "\n") ++ 
    (mkDefines (n+1) ss) -- "nsDefine inPackage s" not needed 
                         -- (see cf2flex::makeSymEnv)
  ...
  mkFuncs s | normCat s == s = identCat s ++ "*" +++ "p" ++ identCat s ++
    "(FILE *inp);\n" ++ identCat s ++ "*" +++ "p" ++ identCat s ++
    "(const char *str);\n"
  mkFuncs _ = ""
