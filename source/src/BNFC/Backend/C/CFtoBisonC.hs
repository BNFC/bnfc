...
header mode cf = unlines $ concat [
  ...
  , when (and (beyondAnsi mode, isJust ns))
    [ "%define api.namespace {" ++ nsString ns ++ "}"
  ...
  , if beyondAnsi mode then
      ...
      , "    " ++ nsEnd ns
      , "}"
      , "%parse-param { " ++ nsScope ns ++ camelCaseName ++ "Scanner  &scanner  }"
      , "%parse-param { " ++ nsScope ns ++ camelCaseName ++ "Driver  &driver  }"
      , ""
